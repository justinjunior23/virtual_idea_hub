...
from django.conf import settings  # ✅ FIXED: was missing in previous version
...
ALLOWED_MIMETYPES = None  # set to a list to whitelist, or None to allow all


def _save_attachments(request, category_slug: str, post_id: int):
    """
    Iterate over request.FILES.getlist('attachments') and create one
    PostAttachment per file.  Call this right after creating a post.

    Returns (saved_count, skipped_count, error_messages).
    """
    ...
    # ✅ FIXED: read from settings (already in bytes); removed erroneous * 1024 * 1024
    max_size_bytes = settings.MAX_UPLOAD_SIZE
    max_size_mb    = settings.MAX_FILE_SIZE_MB

    for f in files:
        if f.size > max_size_bytes:
            errors.append(f"'{f.name}' exceeds the {max_size_mb} MB limit and was skipped.")
            skipped += 1
            continue

        file_type = PostAttachment.detect_file_type(f.name)
...
def staff_bulk_update(request, category_slug):
    """Allow staff to mark multiple submissions at once."""
...
def create_awareness(request):
    if request.method == 'POST':
        ...
        if title and content:
            post = PostAwareness.objects.create(
            ...
            saved, skipped, errors = _save_attachments(request, 'awareness', post.pk)
            for err in errors:
...
def create_innovation(request):
    if request.method == 'POST':
        ...
        anonymous = request.POST.get('post_as_anonymous') == 'on'
        if title and content:
            post = PostInnovation.objects.create(
                user         = None if anonymous else (request.user if request.user.is_authenticated else None),
                title        = title,
                content      = content,
                is_anonymous = anonymous or not request.user.is_authenticated,
            )
            _save_attachments(request, 'innovation', post.pk)
        return _post_submit_redirect(request)
...
def create_suggestion(request):
    if request.method == 'POST':
        ...
        anonymous = request.POST.get('post_as_anonymous') == 'on'
        if title and content:
            post = PostSuggestions.objects.create(
                user         = None if anonymous else (request.user if request.user.is_authenticated else None),
                title        = title,
                content      = content,
                is_anonymous = anonymous or not request.user.is_authenticated,
            )
            _save_attachments(request, 'suggestion', post.pk)
        return _post_submit_redirect(request)
...
def create_emergency(request):
    if request.method == 'POST':
        ...
        anonymous = request.POST.get('post_as_anonymous') == 'on'
        if title and content:
            post = PostEmergency.objects.create(
                user         = None if anonymous else (request.user if request.user.is_authenticated else None),
                title        = title,
                content      = content,
                is_anonymous = anonymous or not request.user.is_authenticated,
            )
            _save_attachments(request, 'emergency', post.pk)
        return _post_submit_redirect(request)
...
def create_recommendation(request):
    if request.method == 'POST':
        ...
        anonymous = request.POST.get('post_as_anonymous') == 'on'
        if title and content:
            post = PostRecommendations.objects.create(
                user         = None if anonymous else (request.user if request.user.is_authenticated else None),
                title        = title,
                content      = content,
                is_anonymous = anonymous or not request.user.is_authenticated,
            )
            _save_attachments(request, 'recommendation', post.pk)
        return _post_submit_redirect(request)
...
def create_complain(request):
    if request.method == 'POST':
        ...
        anonymous = request.POST.get('post_as_anonymous') == 'on'
        if title and content:
            post = PostComplain.objects.create(
                user         = None if anonymous else (request.user if request.user.is_authenticated else None),
                title        = title,
                content      = content,
                is_anonymous = anonymous or not request.user.is_authenticated,
            )
            _save_attachments(request, 'complaint', post.pk)
        return _post_submit_redirect(request)
...
def create_others(request):
    if request.method == 'POST':
        ...
        anonymous = request.POST.get('post_as_anonymous') == 'on'
        if title and content:
            post = PostOthers.objects.create(
                user         = None if anonymous else (request.user if request.user.is_authenticated else None),
                title        = title,
                content      = content,
                is_anonymous = anonymous or not request.user.is_authenticated,
            )
            _save_attachments(request, 'others', post.pk)
        return _post_submit_redirect(request)
...
def post_reporting(request):
    if request.method == 'POST':
        ...
        _save_attachments(request, 'reporting', report.pk)
        return _post_submit_redirect(request)
...
def notifications_partial(request):
    ...
    ctx['resolved_today']      = StaffFeedback.objects.filter(created_at__date=timezone.now().date()).count()
...
def _workflow_partial(request, status_key, title, desc, color):
    ...
    ctx.update({
        ...
        'workflow_week':     week,
        'workflow_sub':      'Across all categories',
        'stat_color':        color,
